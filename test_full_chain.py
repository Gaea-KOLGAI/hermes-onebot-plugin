...
try:
    ...
    module_files = {
        ...
        "TransportLifecycle": base_dir / "onebot_platform" / "transport" / "lifecycle.py",
        "MessageMixin": base_dir / "onebot_platform" / "inbound" / "message_mixin.py",
        "InboundFiles": base_dir / "onebot_platform" / "inbound" / "files.py",
        "InboundContext": base_dir / "onebot_platform" / "inbound" / "context.py",
    ...
    checks = {
        ...
        "TransportLifecycle": module_files["TransportLifecycle"].exists(),
        "MessageMixin": module_files["MessageMixin"].exists(),
        "InboundFiles": module_files["InboundFiles"].exists(),
        "InboundContext": module_files["InboundContext"].exists(),
...
try:
    ...
    assert hasattr(bot, "_in_edit_resend_count"), "应使用 _in_edit_resend_count 计数器"
    assert not hasattr(bot, "_in_edit_resend_chats"), "不应使用 _in_edit_resend_chats set"
    assert isinstance(bot._in_edit_resend_count, dict), "计数器应初始化为 dict"
