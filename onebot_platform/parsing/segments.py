...
def _segments_text(segments: List[Dict[str, Any]]) -> str:
    ...
    for seg in segments:
        ...
        if seg_type == "text":
            result.append(_cq_unescape(str(data.get("text", ""))))
        elif seg_type == "at":
            qq = str(data.get("qq", ""))
            ...
            if qq == "all":
                ...
                result.append(f"@{name}(QQ:{qq})")
            elif qq:
                result.append(f"@{qq}")
        elif seg_type == "face":
            fid = data.get("id", "")
            result.append(f"[表情:id={fid}]")
        elif seg_type == "image":
            url = data.get("url", "") or data.get("file", "")
            result.append(f"[图片:url={url}]")
        elif seg_type == "record":
            url = data.get("url", "") or data.get("file", "")
            result.append(f"[语音:url={url}]")
        elif seg_type == "video":
            url = data.get("url", "") or data.get("file", "")
            result.append(f"[视频:url={url}]")
        elif seg_type == "file":
            name = data.get("name", "") or data.get("file", "")
            url = data.get("url", "") or data.get("file_id", "")
            result.append(f"[文件:name={name},url={url}]")
