...
class SendMixin:
    ...
    async def _should_quote(self, chat_id: str, reply_to: Optional[str]) -> Optional[str]:
        if not reply_to:
            return None
        settings = self._plugin_settings.get_chat(chat_id)
        if settings.get("quote_reply") is False:
            return None
    ...
    async def _message_with_optional_reply(self, chat_id: str, reply_to: Optional[str], *segments: dict) -> List[dict]:
        message = []
        quoted = await self._should_quote(chat_id, reply_to)
        if quoted:
    ...
    def _should_auto_forward_text(self, chat_id: str, content: str, reply_to: Optional[str]) -> bool:
        if reply_to:
            return False
        settings = self._plugin_settings.get_chat(chat_id)
        if settings.get("auto_forward") is False:
            return False
        try:
            msg_kind, _target_id = _parse_chat_id(chat_id)
        except Exception:
    ...
    def _smart_split(text: str, max_len: int) -> List[str]:
        """Split text at paragraph/sentence boundaries, falling back gracefully."""
        if len(text) <= max_len:
            return [text]
        chunks: List[str] = []
        remaining = text
        while len(remaining) > max_len:
            ...
            if pos < max_len * 0.3:
                pos = remaining.rfind("\n", 0, max_len)
            # 3. Chinese/English sentence end
            if pos < max_len * 0.3:
                for punct in ("。", "！", "？", "；", ".", "!", "?", "…"):
                    p = remaining.rfind(punct, 0, max_len)
                    if p > 0:
                        pos = p + 1
                        break
            # 4. space
    ...
    async def send(
        ...
        if auto_forward is not None:
            return auto_forward
        message_segments = await self._message_with_optional_reply(
