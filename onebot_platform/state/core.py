...
class DedupCache:
    def __init__(self, ttl: float, max_size: int):
        self._ttl = _num(ttl, 60.0, float, 0.0)
        self._max_size = _num(max_size or 1, 1, int, 1)
...
class RateLimiter:
    def __init__(self, rate: float, burst: int):
        self._rate = _num(rate, 1.0, float, 0.001)
        self._burst = _num(burst, 1.0, float, 1.0)
...
class MemberCache:
    def __init__(self, ttl: float = 300):
        self._ttl = _num(ttl, 300.0, float, 0.0)
        self._max_size = 5000
...
class _MediaCache:
    ...
    async def download(self, url: str, http_client, media_type: str = "image") -> Optional[str]:
        ...
        if not await asyncio.to_thread(self._is_safe_media_download_url, url):
            return None
        import httpx
