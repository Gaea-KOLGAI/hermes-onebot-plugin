...
def test_group_upload_notice_dispatches_to_handler():
    class UploadAdapter(OneBotAdapter):
        def __init__(self):
    ...
    data = {
        ...
        "group_id": 1000000001,
        "user_id": 100000001,
        "file": {"name": "report.txt", "size": 1024},
    }
    asyncio.run(bot._handle_notice(data, bot._default_conn))
    assert bot.group_upload_calls == 1
    assert bot.last_data == data
...
def test_send_action_success_cleans_echo_state(monkeypatch):
    ...
    monkeypatch.setattr("onebot_platform.outbound.send_mixin.asyncio.wait_for", fast_wait)
