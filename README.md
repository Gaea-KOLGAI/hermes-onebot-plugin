...
Hermes Agent 的 OneBot v11 平台适配器，主要面向 NapCat QQ 场景。它负责把 QQ 私聊、群聊、图片、语音、视频、文件、引用、戳一戳等事件接入 Hermes，同时支持把 Hermes 的回复、媒体和审批提示发回 QQ。
...
- SSRF 防护：解析域名后检查 IP 网段，阻止本地和内网地址
- 路径穿越防护：所有本地路径都经过 resolve 校验
- 文件注入白名单：只读允许目录内的小文件
- Token 校验：使用常量时间比较，降低时序侧信道风险
- 日志清理：移除 ANSI 控制符、控制字符和双向文本覆盖符
- 并发限制：每个聊天最多同时 5 个任务
