...
本仓库是纯 AI 生成与维护的项目，无人工手写代码。人工仅负责提出需求、审查结果和运行验证；使用前请自行阅读、测试并承担风险。

## 主要功能

- 私聊和群聊消息收发
- 群聊默认要求 @机器人 唤醒；OneBot 管理指令统一使用 `/onebot ...` 入口
- 图片、语音、视频、文件的接收与发送
...
- 支持 Markdown 清理开关
- 支持私聊戳一戳审批危险命令，群聊审批需回复审批提示或点击提示表情
- 支持给审批提示消息贴表情批准当前会话
- 支持管理员专属审批
- 支持输入状态提示
- 支持消息编辑回退发送
...
反向 WebSocket 模式下，`ws_url` 表示 Hermes 监听地址。若监听 `0.0.0.0` 或其它非本机地址，必须配置 `access_token` / `ONEBOT_ACCESS_TOKEN`，NapCat 连接时也必须携带 `Authorization: Bearer <token>`；无 token 只允许 loopback/本机连接。
...
多账号模式下，聊天 ID 会带账号前缀，例如 `main:private_123456` 或 `work:group_123456`。`accounts[].home_channel` 建议填写该账号自己的原始聊天 ID，例如 `private_123456` 或 `group_123456`；跨平台发送、cron 投递或手动指定目标时，建议使用带账号前缀的完整 chat_id。若显式账号前缀不存在，插件会 fail-closed，拒绝回退到默认账号，避免错账号发送。
...
| `/setfilemode on|off` | 群聊文件发私聊模式（图片/文件直接发给最近发言者） | 是 |
...
群聊中使用 `/setfilemode on` 开启后，所有图片和文件会直接发给最近发言者的私聊，而非群聊。文字消息不受影响，仍然发在群里。使用 `/setfilemode off` 关闭（默认关闭）。该开关是 per-chat 设置，不同群互不影响，切换即时生效。
...
- 私聊戳一戳机器人：单次批准
- 给审批提示消息贴表情：会话批准
- 私聊直接回复 `1`，或群聊回复审批提示消息 `1`：单次批准
- 私聊直接回复 `2`，或群聊回复审批提示消息 `2`：会话批准
- 私聊直接回复 `3`，或群聊回复审批提示消息 `3`：永久批准
- 私聊直接回复 `4`，或群聊回复审批提示消息 `4`：拒绝

群聊不会用戳一戳批准危险命令，避免任何群成员误触；群聊文本审批必须回复审批提示消息，表情审批也必须作用在审批提示消息上。这个映射与 Hermes 审批系统的 `once`、`session`、`always`、`deny` 对齐。
...
- Token 校验：使用常量时间比较，降低时序侧信道风险；反向 WebSocket 监听非 loopback 地址时必须配置 `access_token`，NapCat 连接时需携带 Authorization 头，值为 Bearer 加同一个 access token
- 并发限制：每个聊天最多同时 5 个任务，并按 chat 保存完整任务集合，断开连接时会批量取消
...
| `ONEBOT_ACCESS_TOKEN` | NapCat 访问令牌；反向 WebSocket 监听非 loopback 地址时必须配置 | 非本机反向监听必需 |
...
| `ONEBOT_ADMIN_QQ` | 管理员 QQ 号；不会自动回退到白名单第一位 | 管理指令需要 |
...
- 收不到群消息：确认群号在白名单内；`allow_all` 只放宽用户，不会绕过 `group_ids` 群范围
- 群聊不响应：普通消息和未知 slash 都需要 @ 机器人；OneBot 插件管理命令用 `/onebot ...`
- 图片发不出去：配置 HTTP API 作为兜底通道
- 反向 WebSocket 不稳定：检查 NapCat 是否已连回 Hermes；非本机监听必须配置相同 access_token
- 管理指令无效：确认 `ONEBOT_ADMIN_QQ` 或 `admin_qq` 已显式配置为当前 QQ
