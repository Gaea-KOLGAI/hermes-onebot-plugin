...
插件现在是模块化适配器。根目录 `adapter.py` 只作为兼容入口，继续保留历史导入面；真实实现放在 `onebot_platform/` 包内。

| 模块 | 职责 |
|---|---|
| `onebot_platform.adapter` | 主适配器组合类、Hermes 平台接口、兼容导出 |
| `onebot_platform.adapter_runtime` | Hermes/Gateway 运行时公共依赖与常量 |
| `onebot_platform.state` | 去重、限流、成员缓存、媒体缓存、连接状态、持久设置 |
| `onebot_platform.config` | 环境变量、YAML 配置、Gateway 工具提示开关、路径构造 |
| `onebot_platform.parsing` | CQ/消息段解析、媒体段提取、chat_id 构造解析 |
| `onebot_platform.security` | SSRF 与本地文件路径安全检查 |
| `onebot_platform.transport` | WebSocket 兼容连接、鉴权头、连接生命周期 |
| `onebot_platform.inbound` | 入站消息处理、授权、唤醒、媒体下载、文件注入 |
| `onebot_platform.outbound` | 文本、图片、语音、视频、文件、转发等发送能力 |
| `onebot_platform.commands` | QQ 内 `/onebot` 管理指令和帮助信息 |
| `onebot_platform.gateway_integration` | 危险命令审批、更新确认、戳一戳批准 |

根目录仍保留 `adapter.py`、`state.py`、`message_utils.py`、`config_utils.py` 等薄兼容壳，确保旧测试和外部 `from adapter import ...` 用法不失效。
...
适配器当前已拆成 `onebot_platform/` 功能包，根目录兼容壳保持旧导入可用。更新后需至少通过 `py_compile`、`test_full_chain.py` 和 `test_optimizations.py`。
